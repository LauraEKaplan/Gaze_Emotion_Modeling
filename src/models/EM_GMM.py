...
def GMM_fit(x_ND, m, s, K, max_iter) :
    ...
    return log_pi_K, mu_KD, stddev_KD, score_train_history, loss_train_history
...
def run_loocv(x_ND, Y, m, s, K, max_iter):
    ...
    for i in range(N):
        X_train = np.delete(x_ND, i, axis=0)
        Y_train = np.delete(Y, i, axis=0)

        # Create joint X and Y for training
        XY = np.hstack([X_train, Y_train[:, None]])
        X_valid = x_ND[i:i+1]
        Y_valid = Y[i:i+1]


        # Fit model on joint X, Y
        log_pi_K, mu_KDp1, stddev_KDp1, score_train_history, loss_train_history = GMM_fit(XY, m, s, K, max_iter)
        std_X = stddev_KDp1[:, :D]
        std_Y = stddev_KDp1[:, D]
        mu_X = mu_KDp1[:, :D]
        mu_Y = mu_KDp1[:, D]
        ...
        r_NK_predict = responsibilities_for_x(X_valid, log_pi_K, mu_X, std_X)

        # Predict
        Y_pred = np.sum(mu_Y * r_NK_predict)

        loocv_scores.append(calc_score(Y_valid, Y_pred))

        # score_histories.append(np.array(score_train_history))
        # loss_histories.append(np.array(loss_train_history))

    # for one_loss_list in score_histories:
    #     plt.plot(one_loss_list)
    # plt.show() 

    return sum(loocv_scores)/N

def run_kfcv(x_ND, Y, m, s, K, cv_K, max_iter, beta = 10):
    N, D = x_ND.shape
    ...
    for train_index, val_index in kf.split(x_ND):
        ...
        for one_X in X_valid :
            ...
            # Get responsibilities
            r_NK_predict = responsibilities_for_x(one_X, log_pi_K, mu_X, std_X)
        
            # Predict
            Y_pred.append(np.sum(mu_Y * r_NK_predict))


        loocv_scores.append(calc_score(Y_valid, np.array(Y_pred), beta))

    return sum(loocv_scores)/cv_K

def tune_hyperparameters(x_ND, Y, cv_K, max_iter, beta, m_list = [0.5,1,2], s_list = [3,10,25, 50], k_list = [2,3,5,7,10,15]) :
    best_m = -1
    best_s = -1
    best_k = -1
    best_score = -1
    for m in m_list :
        for s in s_list :
            for k in k_list :
                score = run_kfcv(x_ND, Y, m, s, k, cv_K, max_iter, beta)
                if score > best_score :
                    best_score = score
                    best_m = m
                    best_s = s
...
def train_and_eval(X_train, Y_train, X_test, Y_test, m, s, K, max_iter, beta = 10):
    N, D = X_train.shape
    ...
    print(f"Score: {calc_score(Y_test, np.array(Y_pred), beta)}")
...
for emotion in ["Joy","Happiness","Calmness","Relaxation","Anger","Disgust","Fear","Anxiousness","Sadness"] :
    ...
    best_m, best_s, best_k, best_score = tune_hyperparameters(X_train, y_train, 5, 4, beta)
    ...
    train_and_eval(X_train, y_train, X_test, y_test, best_m, best_s, best_k, 4, beta)
    print()